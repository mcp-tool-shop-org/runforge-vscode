...
- [ ] `[cli|mcp|desktop]` SKIP: VS Code extension, not CLI/MCP/desktop standalone
- [ ] `[cli|mcp|desktop]` SKIP: VS Code extension — file ops constrained to workspace by VS Code sandbox
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[mcp]` SKIP: not an MCP server

## B. Error Handling

- [x] `[all]` Errors follow the Structured Error Shape: `code`, `message`, `hint`, `cause?`, `retryable?` (2026-02-27) — SafeError with code, message, recoveryHint, retryable, originalError
- [ ] `[cli]` SKIP: not a CLI tool
- [ ] `[cli]` SKIP: not a CLI tool
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[desktop]` SKIP: VS Code extension, not standalone desktop
- [x] `[vscode]` Errors surface via VS Code notification API — no silent failures (2026-02-27) — all commands use vscode.window.showErrorMessage
...
- [ ] `[cli]` SKIP: not a CLI tool
- [ ] `[cli|mcp|desktop]` SKIP: VS Code extension — logging handled by VS Code Output Channel
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[complex]` SKIP: not complex enough for ops handbook — has TRUST_MODEL.md and CONTRACT.md instead
...
- [ ] `[desktop]` SKIP: not a standalone desktop app
